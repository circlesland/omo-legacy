...
export class ModelQuant
{
  ...
  async addSubscriptionResolver(subscription: any, quantRegistry: QuantRegistry)
  {
    ...
    //     subscribe: async (_, { _id }) => { console.warn("Subscription not working in the moment"); return ModelQuant.pubsub.asyncIterator(_id + "_changed") },
    // }
    // subscription[this.typeName(this.name) + 'Added'] = {
    //     resolve: async (_, data) => { console.log(data); return await collection.find({}) },
    //     subscribe: async () => { console.warn("Subscription not working in the moment"); return ModelQuant.pubsub.asyncIterator(this.collectionName + "_added") },
    // }
    subscription[this.typeName(this.name) + 'Updated'] = {
      resolve: async (_: any, data: any) =>
      {
        console.log("Trying to resolve " + this.typeName(this.name) + 'Updated: ', _, data);
        return await collection.findById(_.id);
      },
      subscribe: async () =>
      {
        if (!ModelQuant.subscriptions.some(x => x.collection == this.collectionName + "_updated"))
        {
          ModelQuant.subscriptions.push({collection: this.collectionName + "_updated"});

          collection.observeUpdate(["save"], "", (foo: any) =>
          {
            console.log(`${this.collectionName}_updated`, foo);
            ModelQuant.pubsub.publish(this.collectionName + "_updated", foo);
          });
          console.log(`${this.collectionName}_updated subscribed`);
        }

        return ModelQuant.pubsub.asyncIterator(this.collectionName + "_updated")
      },
    }
    // subscription[this.typeName(this.name) + 'Deleted'] = {
    //     resolve: async (_, data) => { console.log(data); return await collection.find({}) },
    //     subscribe: async () => { console.warn("Subscription not working in the moment"); return ModelQuant.pubsub.asyncIterator(this.collectionName + "_deleted") },
    // }
  }
