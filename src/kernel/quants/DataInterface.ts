...
export default class DataInterface extends Quant {
  ...
  public static async registerSchemaIfNoRegistered(jsonSchema: any, modelName: string): Promise<void> {
    try {
      await window.omo.client.registerSchema(window.omo.storeId, modelName, jsonSchema);
    }
    catch (err) {
  ...
  public static async getModelName(jsonSchema: any): Promise<string> {
    ...
    const hashJsonSchemaResult = await window.omo.ipfs.add(JSON.stringify(jsonSchema), { onlyHash: true });
