...
export default class ThreadsClientInterface {
    ...
    public async  modelFindByID<T = any>(storeID: string, modelName: string, entityID: string): Promise<Entity<T>> {
        return this.server.modelFindByID<T>(storeID, modelName, entityID)
    }

    public listen<T = any>(storeID: string, modelName: string, entityID: string, callback: (reply: Entity<T>) => void): void {
        this.server.listen(storeID, modelName, entityID, callback);
    }

    public async modelSave(storeID: string, modelName: string, values: any[]): Promise<void> {
        this.server.modelSave(storeID, modelName, values);
    }

    public modelCreate<T = any>(storeID: string, modelName: string, values: any[]): Promise<EntityList<T>> {
        return this.server.modelCreate(storeID, modelName, values);
    }

    public async registerSchema(storeID: string, name: string, schema: any): Promise<void> {
        try {
    ...
    public async modelDelete(storeID: string, modelName: string, entityIDs: string[]): Promise<void> {
        return this.server.modelDelete(storeID, modelName, entityIDs);
    }

    public async modelHas(storeID: string, modelName: string, entityIDs: string[]): Promise<boolean> {
