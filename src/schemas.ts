import { GraphQLID, GraphQLList, GraphQLObjectType, GraphQLSchema, GraphQLString, GraphQLScalarType, GraphQLBoolean } from 'graphql';
import { PubSub } from 'graphql-subscriptions';
import { Result } from 'interface-datastore';
import { AuthorCollection, BookCollection, LibraryCollection } from './textileThreads';
...
export const AuthorType = new GraphQLObjectType({
    fields: () => ({
        ID: { type: GraphQLID },
        books: {
            resolve: async (parent: Author) => toArray(await BookCollection.find({ authorId: parent.ID })),
...
export const LibraryType = new GraphQLObjectType({
    fields: () => ({
        ...
        books: {
            resolve: async (parent: Library) => toArray(await BookCollection.find({ libraryId: parent.ID })),
            type: GraphQLList(BookType),
        },
...
export const BookType = new GraphQLObjectType({
    fields: () => ({
        ID: { type: GraphQLString },
        author: {
            resolve: async (parent: Book) => toArray(await AuthorCollection.find({ ID: parent.authorId })),
            type: GraphQLList(AuthorType),
        },
        library: {
            resolve: async (parent: Book) => toArray(await LibraryCollection.find({ ID: parent.libraryId })),
            type: GraphQLList(BookType),
        },
    ...
    mutation: new GraphQLObjectType({
        fields: {
            ...
            addBook: {
                ...
                resolve: async (root: any, { title, authorId, libraryId }) => {
                    const book = new BookCollection({ title, authorId, libraryId });
                    await book.save();
                    return book;
                },
                type: BookType,
            ...
            addLibrary: {
                ...
                resolve: async (root: any, { branch }) => {
                    const library = new LibraryCollection({ branch });
                    await library.save();
                    return library;
                },
                type: BookType,
            ...
            deleteAuthor: {
                ...
                resolve: async (root: any, { ID }) => {
                    await AuthorCollection.delete(ID).then(() => true).catch(() => false);
                },
                type: GraphQLBoolean,
            },
            deleteBook: {
                ...
                resolve: async (root: any, { ID }) => {
                    await BookCollection.delete(ID).then(() => true).catch(() => false);
                },
                type: GraphQLBoolean,
            },
            deleteLibrary: {
                ...
                resolve: async (root: any, { ID }) => {
                    await LibraryCollection.delete(ID).then(() => true).catch(() => false);
                },
                type: GraphQLBoolean,
            }
        },
